...
const DishSelect = ({
	...
	return (
		<li ref={liRef} className="">
			<div
				className="list-row shadow"
				onClick={() => hasVariants && setShowVariants((p) => !p)}
			>
				<div>
					<img
						className="size-10 rounded-box"
						src={`https://res.cloudinary.com/dvniua4ab/image/upload/c_thumb,g_center,h_150,w_150/f_webp/q_auto:eco/${dish.main_dishes.image}`}
						alt=""
					/>
				</div>
					...
					<>
						<button
							...
							onClick={() =>
								hasVariants
									...
									: handleSelect(dish.main_dishes.title, "dec")
							}
						...
						<button
							...
							onClick={() =>
								hasVariants
									? setShowVariants(true)
									: handleSelect(dish.main_dishes.title, "inc")
							}
							className="btn btn-square "
						>
							&#x2B;
				...
				>
					<li className="list-row justify-between flex">
						...
						<div>
							<button
								type="button"
								onClick={() => handleSelect(dish.main_dishes.title, "dec")}
								className="btn btn-square"
							>
								&minus;
							</button>
							<span className="content-center mx-2">
								{selection?.[dish.main_dishes.title] ?? 0}
							</span>
							<button
								type="button"
								onClick={() => handleSelect(dish.main_dishes.title, "inc")}
								className="btn btn-square"
							>
								&#x2B;
							</button>
