...
const Home = () => {
  ...
  const changeSelection = useCallback((name: string, operation: 'inc' | 'dec') => {
    setSelection((prev) => {
  ...
  return (
    <>
      ...
      {selection && (
        <>
          <button type='button' onClick={() => setSelection(null)} className='fixed top-3 left-5 z-50 btn-warning btn '>
            Reset
          </button>
          <button type='button' onClick={handleConfirm} className='fixed top-3 right-5 z-50 btn-primary btn'>
            In die Küche!
          </button>
        </>
      )}
      ...
      <dialog ref={dialogRef} className='modal'>
        <div className='modal-box'>
          ...
          <div className='modal-action flex justify-between'>
            ...
            <button className='btn btn-success' type='button' onClick={handleSendToKitchen}>
              In die Küche
            </button>
          </div>
        </div>
